...
type tableWriterBase struct {
	...
	// deadlockTimeout specifies the maximum amount of time that the writer will
	// wait while attempting to acquire a lock on a key.
...
func (tb *tableWriterBase) init(
	...
	if evalCtx != nil {
		...
		//fmt.Println("!!! IBRAHIM !!! evalCtx.SessionData().DeadlockTimeout =", evalCtx.SessionData().DeadlockTimeout)
