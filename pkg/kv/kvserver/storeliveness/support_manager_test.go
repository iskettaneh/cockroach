...
func TestSupportManagerRequestsSupport(t *testing.T) {
	...
	sm := NewSupportManager(store, engine, options, settings, stopper, clock, nil, sender, nil, nil)
...
func TestSupportManagerRestart(t *testing.T) {
	...
	sm = NewSupportManager(store, engine, options, settings, stopper, clockBehind, nil, sender, nil, nil)
	now := sm.clock.Now()
