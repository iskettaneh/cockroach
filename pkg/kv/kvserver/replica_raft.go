...
// leader. Forced elections cause other voters to ignore the leader's
// lease and vote. Currently, it's only used in the case where a leader is
...
// leaseholder is the one calling this method, so there shouldn't be any
// operations running on the range at this point. Ideally, we only want the
// leader to tell a follower to campaign, and not let a follower call it by
// itself.
//
