...
func runSysbench(ctx context.Context, t test.Test, c cluster.Cluster, opts sysbenchOptions) {
	...
	runWorkload := func(ctx context.Context) error {
		...
		stmt1 := `SET CLUSTER SETTING kv.closed_timestamp.follower_reads.override_force.enabled = true`
		runner.Exec(t, stmt1)
		t.L().Printf(`executed extra setup statement: %s`, stmt1)

		stmt2 := `SET CLUSTER SETTING kv.closed_timestamp.follower_reads.force.enabled = true`
		runner.Exec(t, stmt2)
		t.L().Printf(`executed extra setup statement: %s`, stmt2)
